...
int show_custom_dialog(const char *title, const char *msg,
  ...
  Fl_Window *win = new Fl_Window(440, 210, "Radian");
  win->color(C_BG);
  win->set_modal();
...
string get_config_path() {
  ...
  if (xdg_config) {
    config_dir = string(xdg_config) + "/radian";
  } else {
    const char *home = getenv("HOME");
    if (home) {
      config_dir = string(home) + "/.config/radian";
    } else {
      config_dir = ".";
    }
  }
...
void show_permission_guide() {
  const char *msg = "Radian needs access to input devices.\n\n"
                    "Run this command in terminal:\n"
                    "  sudo usermod -aG input $USER\n\n"
                    "Then log out and log back in.\n\n"
                    "For details, visit:\n"
                    "github.com/diabloget/radian";

  int result =
      show_custom_dialog("Setup Required", msg, "Copy Command", "Exit");

  if (result == 1) {
    const char *cmd = "sudo usermod -aG input $USER";
...
int main(int argc, char **argv) {
  ...
  bool has_input = check_input_access();
  bool has_uinput = check_uinput_access();

  if (!has_input || !has_uinput) {
  ...
  win->xclass("io.github.diabloget.radian");
  win->color(C_BG);
